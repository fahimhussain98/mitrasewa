...
public class SettlementActivity extends AppCompatActivity {
    ...
    ImageView imgBack;
    EditText etAmount, etAccountHolderName, etAccountNumber, etIfscNumber, etUpiId;
    Button btnSubmit,btnAddMoreBanks;
    SharedPreferences sharedPreferences;
    String amount, upiId, selectedAccountHolderName, bankName, selectedAccountNumber, selectedIfscNumber, panCard, mobileNo;
    SimpleDateFormat webServiceDateFormat;
    String currentDate;
    String deviceId, deviceInfo;
    String payoutType = "", lat, longi;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        payoutType = getIntent().getStringExtra("payoutType");
...
//                            }
    ...
    private void doSettlement() {
        ...
        if (payoutType.equalsIgnoreCase("payout1"))
        {
             call = RetrofitClient.getInstance().getApi().moveToBank(AUTH_KEY, userId, deviceId, deviceInfo, selectedPaymentType, amount, selectedAccountNumber,
                    selectedIfscNumber, selectedAccountType, selectedTransactionType, selectedAccountHolderName, selectedBankName, upiId);
        } else if (payoutType.equalsIgnoreCase("payout2")) {
            call = RetrofitClient.getInstance().getApi().moveToBank2(AUTH_KEY, userId, deviceId, deviceInfo, selectedPaymentType, amount, selectedAccountNumber,
                    selectedIfscNumber, selectedAccountType, selectedTransactionType, selectedAccountHolderName, selectedBankName, upiId, lat, longi);
        } else
        {
            call = RetrofitClient.getInstance().getApi().moveToBank3(AUTH_KEY, userId, deviceId, deviceInfo, selectedPaymentType, amount, selectedAccountNumber,
                    selectedIfscNumber, selectedAccountType, selectedTransactionType, selectedAccountHolderName, selectedBankName, upiId, lat, longi, "6498");
        }
                ...
                if (response.isSuccessful()) {
                    try {
                        ...
                        if (responseCode.equalsIgnoreCase("TXN") ) {
                            ...
                            Intent intent=new Intent(SettlementActivity.this, SharePayoutReportActivity.class);
                            ...
                            intent.putExtra("isPayoutReport",false);
                            ...
                            new AlertDialog.Builder(SettlementActivity.this).setTitle("Message")
                                    .setMessage(message)
                                    .setCancelable(false)
                                    .setPositiveButton("OK", new DialogInterface.OnClickListener() {
                                        @Override
    ...
    private boolean checkInputs() {
        ...
        if (!selectedPaymentType.equalsIgnoreCase("select")) {
            ...
            if (selectedPaymentType.equalsIgnoreCase("WALLET")) {
                ...
                return !TextUtils.isEmpty(etAmount.getText()) && !TextUtils.isEmpty(etUpiId.getText());
            }
        ...
        HintSpinner<String> hintSpinner = new HintSpinner<>(spinnerPaymentType, new HintAdapter<String>(SettlementActivity.this, "Payment Mode", paymentTypeList),
                new HintSpinner.Callback<String>() {
                    ...
                    public void onItemSelected(int position, String itemAtPosition) {
                        ...
                        } else if (selectedPaymentType.equalsIgnoreCase("UPI")) {
                            bankDetailsContainer.setVisibility(View.GONE);
